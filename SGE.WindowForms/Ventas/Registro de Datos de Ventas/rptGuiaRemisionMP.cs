...
{
    public partial class rptGuiaRemisionMP : DevExpress.XtraReports.UI.XtraReport
    {
        public rptGuiaRemisionMP()
        {
            InitializeComponent();
        }
        public void cargar(EGuiaRemision oBe, List<EGuiaRemisionMPDet> mlisdetalle, string Departamento, string Provincia, string Distrito,
            Boolean Ventas, Boolean Compras, Boolean Transformacion, Boolean Consignacion, Boolean Devolucion, Boolean Transferencia, Boolean TransladoEmision, Boolean Otros)
        {
            ...
            if (sobrante > 0)
            {
                lblpuntollegada2.Text = oBe.remic_vdireccion_destinatario.Substring((oBe.remic_vdireccion_destinatario.Length - sobrante), sobrante);
            }
            ...
            lblTotalPares.Text = mlisdetalle.Sum(x => x.dremc_ncantidad_producto).ToString();
            ...
            {
            new XRBinding("Text", mlisdetalle , "dremc_ncantidad_producto", "")
            });
