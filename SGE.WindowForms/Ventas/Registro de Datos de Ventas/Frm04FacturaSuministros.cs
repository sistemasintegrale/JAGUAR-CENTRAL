...
{
    public partial class Frm04FacturaSuministros : DevExpress.XtraEditors.XtraForm
    {
        List<EFacturaCab> lstFacturas = new List<EFacturaCab>();
        bool flag_close = false;
        public string MotivoBaja;
        public Frm04FacturaSuministros()
        {
            InitializeComponent();
        }
        ...
        private void actualizarDetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            lstCab = new BVentas().listarFacturaCab(Parametros.intEjercicio).Where(x => x.favc_tipo_factura == 2).ToList();
            lstCab.ForEach(x =>
            {
