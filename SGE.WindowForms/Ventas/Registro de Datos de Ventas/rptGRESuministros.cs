...
{
    public partial class rptGRESuministros : DevExpress.XtraReports.UI.XtraReport
    {
        public rptGRESuministros()
        {
            InitializeComponent();
        }
        public void cargar(EGuiaRemision oBe, List<EGuiaRemisionMPDet> mlisdetalle)
        {
            ...
            lblTotalPares.Text = Convert.ToInt32(mlisdetalle.Sum(x => x.dremc_ncantidad_producto)).ToString();


            #endregion


            lblItem.DataBindings.AddRange(new XRBinding[] { new XRBinding("Text", mlisdetalle, "strCodProducto", "") });
            lblCantidad.DataBindings.AddRange(new XRBinding[] { new XRBinding("Text", mlisdetalle, "dremc_ncantidad_producto", "") });
            lblUnidad.DataBindings.AddRange(new XRBinding[] { new XRBinding("Text", mlisdetalle, "strDesUM", "") });
            lblTipo.DataBindings.AddRange(new XRBinding[] { new XRBinding("Text", mlisdetalle, "strDesProducto", "") });
