...
{
    public partial class Frm05GuiaRemisionComprasMercaderia : DevExpress.XtraEditors.XtraForm
    {
        List<EGuiaRemisionComprasMercaderia> lstGuiaRemisionCompras = new List<EGuiaRemisionComprasMercaderia>();
        public Frm05GuiaRemisionComprasMercaderia()
        {
            InitializeComponent();
        }
        ...
        private void cargar()
        {
            lstGuiaRemisionCompras = new BCompras().listarGuiaRemisionComprasMercaderia().Where(x => x.grcc_sfecha_grc.Year == Parametros.intEjercicio).ToList();
            #region NumFac
            lstGuiaRemisionCompras.ForEach(x =>
            {
                x.NumFac = "";

                List<EFacturaCompra> lstFC = new BCompras().listarFacCompra(Parametros.intEjercicio).Where(fc => fc.fcoc_icod_documento == x.grcc_icod_grc && fc.fcoc_tipo_documento == 122).ToList();
                if (lstFC.Count > 0)
                {
                    int count = 0;
                    lstFC.ForEach(f =>
        ...
        public void NumeroFac(List<EGuiaRemisionComprasMercaderia> lstGuiaRemisionCompras)
        {
            string NumFac = "";
            lstGuiaRemisionCompras.ForEach(x =>
            {
                List<EFacturaCompra> lstFC = new BCompras().listarFacCompra(Parametros.intEjercicio);
                if (lstFC.Count > 0)
                {
                    lstFC.ForEach(f =>
                    {
        ...
        private void nuevo()
        {
            frmManteGuiaRemisionComprasMercaderia frm = new frmManteGuiaRemisionComprasMercaderia();
            frm.MiEvento += new frmManteGuiaRemisionComprasMercaderia.DelegadoMensaje(reload);
            frm.SetInsert();
            frm.Show();
        }
        ...
        private void eliminar()
        {
            ...
            if (lstGuiaRemisionCompras.Count > 0)
            {
                if (XtraMessageBox.Show("Desea Eliminar el registro", "Informacion del Sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                {
                    ...
                    List<EGuiaRemisionComprasMercaderiaDetalle> LISTDETALLE = new List<EGuiaRemisionComprasMercaderiaDetalle>();
                    EProdKardex ekar = new EProdKardex();
                    List<EProdProducto> oProducto = new List<EProdProducto>();
                    new BCompras().eliminarGuiaRemisionComprasMercaderia(Obe);
                    LISTDETALLE = new BCompras().listarGuiaRemisionComprasMercaderiaDetalle(Obe.grcc_icod_grc);
                    foreach (EGuiaRemisionComprasMercaderiaDetalle fo in LISTDETALLE)
                    {
                        ...
                        ekar.movimiento = 0;
                        ekar.operacion = 3;
                        icod_karde[0] = Convert.ToInt32(fo.grcd_iid_kardex1);
                        icod_karde[1] = Convert.ToInt32(fo.grcd_iid_kardex2);
                        icod_karde[2] = Convert.ToInt32(fo.grcd_iid_kardex3);
                        icod_karde[3] = Convert.ToInt32(fo.grcd_iid_kardex4);
                        icod_karde[4] = Convert.ToInt32(fo.grcd_iid_kardex5);
                        icod_karde[5] = Convert.ToInt32(fo.grcd_iid_kardex6);
                        icod_karde[6] = Convert.ToInt32(fo.grcd_iid_kardex7);
                        icod_karde[7] = Convert.ToInt32(fo.grcd_iid_kardex8);
                        icod_karde[8] = Convert.ToInt32(fo.grcd_iid_kardex9);
                        icod_karde[9] = Convert.ToInt32(fo.grcd_iid_kardex10);
                        talla[0] = Convert.ToInt32(fo.grcd_talla1);
                        talla[1] = Convert.ToInt32(fo.grcd_talla2);
                        talla[2] = Convert.ToInt32(fo.grcd_talla3);
                        talla[3] = Convert.ToInt32(fo.grcd_talla4);
                        talla[4] = Convert.ToInt32(fo.grcd_talla5);
                        talla[5] = Convert.ToInt32(fo.grcd_talla6);
                        talla[6] = Convert.ToInt32(fo.grcd_talla7);
                        talla[7] = Convert.ToInt32(fo.grcd_talla8);
                        talla[8] = Convert.ToInt32(fo.grcd_talla9);
                        talla[9] = Convert.ToInt32(fo.grcd_talla10);
                        for (int j = 0; j <= 9; j++)
                        {
                        ...
                        new BCompras().eliminarGuiaRemisionComprasMercaderiaDetalle(fo);
