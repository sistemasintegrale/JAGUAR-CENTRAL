...
{
    public partial class Frm03MovLibroBancosCompras : DevExpress.XtraEditors.XtraForm
    {
        #region Propiedades

        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Frm03MovLibroBancosCompras));
        ...
        public Frm03MovLibroBancosCompras()
        {
            InitializeComponent();
        }
        ...
        private void FrmMovLibroBancos_Load(object sender, EventArgs e)
        {
            ...
            BSControls.LoaderLook(lkpBanco, new BTesoreria().listarBancos().Where(x => x.bcoc_icod_banco == 36), "bcoc_vnombre_banco", "bcoc_icod_banco", true);
        ...
        private void ctaPorPagar()
        {
            try
            {
                ...
                cuentas.icodBancos = Convert.ToInt32(lkpBanco.EditValue);
                cuentas.icodBancoDet = Convert.ToInt32(lkpCuenta.EditValue);
                cuentas.intIcodModulo = 14;
                cuentas.Show();
                cuentas.SetInsert();
        ...
        private void ingresoVarios()
        {
            try
            {
                ...
                frm.icodBancos = Convert.ToInt32(lkpBanco.EditValue);
                frm.icodBancoDet = Convert.ToInt32(lkpCuenta.EditValue);
                frm.icodTipo = 1680;
                frm.intIcodModulo = 14;
                frm.Show();
                frm.SetInsert();
                ...
                frm.Text = "Mantenimiento Movimiento Caja - Ingresos Varios";
            }
            catch (Exception ex)
            {
                XtraMessageBox.Show(ex.Message, "Información del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void egresoVarios()
        {
            try
            {
                ...
                icod_movimiento = frm.icod_movimiento;
                frm.Text = "Mantenimiento Movimiento Caja - Egresos Varios";
            }
            catch (Exception ex)
            {
                XtraMessageBox.Show(ex.Message, "Información del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void modificarMovimiento(ELibroBancos Obe, bool Process)
        {
            if (Obe != null)
            {
                ...
                if (Obe.iid_motivo_mov_banco == Parametros.intMotivoVarios)
                {
                    ...
                    //frm.lkpTipoDocumento.EditValue = Obe.ii_tipo_doc;
                    //frm.lkpTipoDocumento.Text = Obe.TipoDocumento;
        ...
        private void Carga()
        {
            try
            {
                ...
                lstMovimientos = new BTesoreria().ListarLibroBancosCompras(Parametros.intEjercicio,
                ...
                else
                {
                    if (lkpMes.EditValue != null)
                    {
                        ...
                        ListaSaldoAnterior = new BTesoreria().ListarLibroBancosSaldoAnteriorCompras(Parametros.intEjercicio, Convert.ToInt32(lkpMes.EditValue), Convert.ToInt32(lkpCuenta.EditValue));
                        ListaSaldoDisponible = new BTesoreria().ListarLibroBancosSaldoDisponibleCompras(Parametros.intEjercicio, Convert.ToInt32(lkpMes.EditValue), Convert.ToInt32(lkpCuenta.EditValue));
        ...
        private void imp(ELibroBancos Obe, EVoucherContableCab oBe, List<EVoucherContableDet> lst)
        {
            ...
            reporte.cargar(Obe, oBe, lst.Where(x =>
                Convert.ToInt64(x.ctacc_iid_cuenta_contable_ref) == 0).ToList(), lkpMes.Text, lkpBanco.Text, lkpCuenta.Text, null);
        }
            ...
            try
            {
                ...
                lstMovBancoDet.ForEach(x =>
                {
                    ...
                    else
                    {
                        ...
                        obj_item_CompDet.strTipNroDocumento = String.Format("{0} {1}", obj_Documento.tdocc_vabreviatura_tipo_doc, obj_item_CompDet.vcocd_numero_doc);
                        if (motivo == Parametros.intMotivoCuentasPorCobrar)
                        {
                            if (x.MonedaDXC == 3)
                ...
                if (motivo == Parametros.intMotivoCuentasPorPagar)
                {
                    lstMovBancoDet.ForEach(x =>
                    {
                        ...
                        if (Convert.ToInt32(x.iid_cuenta_contable) == 0 && x.mto_retenido_soles > 0)
                        {
                            ...
                            obj_item_CompDet_R.strTipNroDocumento = String.Format("{0} {1}", obj_Documento.tdocc_vabreviatura_tipo_doc, obj_item_CompDet_R.vcocd_numero_doc);
        ...
        private void CrearVoucherContableBancosVarios(ELibroBancos obj_LibroBanco, List<ELibroBancosDetalle> lstMovBancoDet)
        {
            try
            {
                ...
                lstMovBancoDet.ForEach(x =>
                {
                    ...
                    ETipoDocumento obj_Documento2 = lstDocumento2.Where(d => d.tdocc_icod_tipo_doc == obj_item_CompDet_01.tdocc_icod_tipo_doc).ToList()[0];
                    ...
                    obj_item_CompDet.ctacc_icod_cuenta_contable = Convert.ToInt64(x.iid_cuenta_contable);
                    Lista = mlistCuenta.Where(Ob => Ob.ctacc_icod_cuenta_contable == obj_item_CompDet.ctacc_icod_cuenta_contable).ToList();
                    Lista.ForEach(Obe =>
                    {
                        obj_item_CompDet.strNroCuenta = Obe.ctacc_numero_cuenta_contable;
        ...
        private void imprimirEstadoDeCuentaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (lstMovimientos.Count > 0)
            {
                ...
                rpt01EstadoCuentasCajaSuministros rpt = new rpt01EstadoCuentasCajaSuministros();
                rpt.carga(lstMovimientos, lkpMes.Text, lkpBanco.Text, lkpCuenta.Text);
            }
            else
            {
