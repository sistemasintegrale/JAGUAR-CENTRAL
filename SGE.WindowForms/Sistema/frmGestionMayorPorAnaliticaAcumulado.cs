...
{
    public partial class frmGestionMayorPorAnaliticaAcumulado : DevExpress.XtraEditors.XtraForm
    {
        List<EVoucherContableDet> lst01 = new List<EVoucherContableDet>();
        List<EVoucherContableDet> lst02 = new List<EVoucherContableDet>();
        private List<ETipoCambioMensual> lstTipoCambioMensual = new List<ETipoCambioMensual>();
        int intMes = -1;
        int? NullVal, intTipoAnalitica;
        string strAnaliticaInicio, strAnaliticaFin, strMes;

        public frmGestionMayorPorAnaliticaAcumulado()
        {
            InitializeComponent();
        }
        ...
        private void getMayorListas()
        {
            if (bteCuenta.Text == "")
            {
                lst01 = new BContabilidad().listarMayorAnaliticaMensual(Parametros.intEjercicio, intMes, intTipoAnalitica, (strAnaliticaInicio == "") ? null : strAnaliticaInicio, (strAnaliticaFin == "") ? null : strAnaliticaFin, 2).Where(x => x.tablc_iid_tipo_moneda_CB == 2).ToList();
                lst02 = new BContabilidad().listarMayorAnaliticaMensual_2(Parametros.intEjercicio, intMes, intTipoAnalitica, (strAnaliticaInicio == "") ? null : strAnaliticaInicio, (strAnaliticaFin == "") ? null : strAnaliticaFin, 2).Where(x => x.tablc_iid_tipo_moneda_CB == 2).ToList();
            }
            else
            {
        ...
        private void calcularMayor()
        {
            lst01.ForEach(x =>
            {
                ...
                if (x.ctacc_iid_cuenta_contable_saldo_sol != 0 && x.ctacc_iid_cuenta_contable_saldo_dol != 0)
                {
                    x.TipoCambio = Math.Round(Convert.ToDecimal(x.ctacc_iid_cuenta_contable_saldo_sol) / Convert.ToDecimal(x.ctacc_iid_cuenta_contable_saldo_dol), 4);
                }
                else
                    x.TipoCambio = 0;
        ...
        {
            lstTipoCambioMensual = new BGeneral().ListarTipoCambioMensual();
            var Lista = lstTipoCambioMensual.Where(ob => ob.tcamm_iid_anio == Parametros.intEjercicio && ob.mesec_iid_mes == Convert.ToInt32(lkpMeses.EditValue)).ToList();
            Lista.ForEach(obe =>
            {
                txtTipoCambioCompras.Text = obe.tcamm_ntipo_cambio_compra.ToString();
                txtTipoCambioVentas.Text = obe.tcamm_ntipo_cambio_venta.ToString();
            });
        }
