...
{
    public partial class frmAlmacenSaldoInicialMercaderia : DevExpress.XtraEditors.XtraForm
    {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmAlmacenSaldoInicialMercaderia));
        List<EProdKardex> lstKardex = new List<EProdKardex>();

        public frmAlmacenSaldoInicialMercaderia()
        {
            InitializeComponent();
        }
        ...
        private void frmAlamcen_Load(object sender, EventArgs e)
        {
            cargar();
            bteAlmacen.Tag = 5;
            bteAlmacen.Text = "ALMACEN CENTRAL";
            filtrar();
        }

        private void cargar()
        {
            lstKardex = new BCentral().listarAlmacenSaldoInicial(Parametros.intEjercicio);
        ...
        {
            cargar();
            int index = lstKardex.FindIndex(x => x.kardc_iid_correlativo == intIcod);
        ...
        {
            frmMantAlmacenSaldoInicialMercaderia frm = new frmMantAlmacenSaldoInicialMercaderia();
            frm.MiEvento += new frmMantAlmacenSaldoInicialMercaderia.DelegadoMensaje(reload);
        ...
        private void modificarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            EProdKardex Obe = (EProdKardex)viewKardex.GetRow(viewKardex.FocusedRowHandle);
            if (Obe == null)
                return;
            frmMantAlmacenSaldoInicialMercaderia frm = new frmMantAlmacenSaldoInicialMercaderia();
            frm.MiEvento += new frmMantAlmacenSaldoInicialMercaderia.DelegadoMensaje(reload);
        ...
        private void eliminarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (XtraMessageBox.Show("¿Esta seguro que desea eliminar el registro?", "Información del Sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    new BCentral().EliminarKardexPvt(Obe);
                    cargar();
        ...
        {
            grdKardex.DataSource = lstKardex.Where(x => x.iid_almacen == Convert.ToInt32(bteAlmacen.Tag) && x.strProducto.Contains(txtProducto.Text.ToUpper())).ToList();
        }
