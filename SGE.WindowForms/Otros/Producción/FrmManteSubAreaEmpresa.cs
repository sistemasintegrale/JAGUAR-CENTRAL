...
{
    public partial class FrmManteSubAreaEmpresa : DevExpress.XtraEditors.XtraForm
    {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FrmManteSubAreaEmpresa));
        public delegate void DelegadoMensaje(int intIcod);
        public event DelegadoMensaje MiEvento;
        private BSMaintenanceStatus mStatus;
        public ESubAreaEmpresa Obe = new ESubAreaEmpresa();
        public List<ESubAreaEmpresa> lstTabla = new List<ESubAreaEmpresa>();
        public int intTabla = 0;
        public bool flagExterno = false;

        public FrmManteSubAreaEmpresa()
        {
            InitializeComponent();
        }
        ...
        {
            txtCodigo.Text = Obe.aremd_codigo.ToString();
        ...
        private void SetSave()
        {
            ...
            try
            {
                ...
                Obe.aremc_iid_codigo = intTabla;
                Obe.aremd_codigo = Convert.ToInt32(txtCodigo.Text);
                Obe.descripcion = txtDescripcion.Text;
                Obe.Estado = (Convert.ToInt32(lkpSituacion.EditValue) == 1) ? 'A' : 'I';

                if (Status == BSMaintenanceStatus.CreateNew)
                {
                    new BCentral().InsertarSubAreaEmpresa(Obe);
                }
                else if (Status == BSMaintenanceStatus.ModifyCurrent)
                {
                    new BCentral().ModificarSubAreaEmpresa(Obe);
                }
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                if (Flag)
                {
                    if (flagExterno)
                        ...
                        this.MiEvento(Obe.aremd_iid_sub_area_empresa);
                    this.Close();
                }
            }
        }
        ...
        private bool verificarDescripcionUnidadM(string strNombre)
        {
            try
            {
                ...
                if (lstTabla.Count > 0)
                {
                    ...
                    if (Status == BSMaintenanceStatus.ModifyCurrent)
                    {
                        if (lstTabla.Where(x => x.descripcion.Trim() == strNombre.Trim() && x.aremd_iid_sub_area_empresa != Obe.aremd_iid_sub_area_empresa).ToList().Count > 0)
                            exists = true;
                    }
                }
                return exists;
