...
{
    public partial class FrmManteAreaDescripción : DevExpress.XtraEditors.XtraForm
    {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FrmManteAreaDescripción));
        public delegate void DelegadoMensaje(int intIcod);
        public event DelegadoMensaje MiEvento;
        private BSMaintenanceStatus mStatus;
        public EAreaDescripcion Obe = new EAreaDescripcion();
        public List<EAreaDescripcion> lstTabla = new List<EAreaDescripcion>();
        public int intTabla = 0;
        public bool flagExterno = false;

        public FrmManteAreaDescripción()
        {
            InitializeComponent();
        }
        ...
        {
            txtCodigo.Text = Obe.arded_codigo.ToString();
        ...
        private void SetSave()
        {
            ...
            try
            {
                ...
                Obe.arded_codigo = Convert.ToInt32(txtCodigo.Text);
                Obe.descripcion = txtDescripcion.Text;
                Obe.Estado = (Convert.ToInt32(lkpSituacion.EditValue) == 1) ? 'A' : 'I';

                if (Status == BSMaintenanceStatus.CreateNew)
                {
                    new BCentral().InsertarAreaDescripcion(Obe);
                }
                else if (Status == BSMaintenanceStatus.ModifyCurrent)
                {
                    new BCentral().ModificarAreaDescripcion(Obe);
                }
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                if (Flag)
                {
                    if (flagExterno)
                        ...
                        this.MiEvento(Obe.arded_iid_area_descripcion);
                    this.Close();
                }
            }
        }
        ...
        private bool verificarDescripcionUnidadM(string strNombre)
        {
            try
            {
                ...
                if (lstTabla.Count > 0)
                {
                    ...
                    if (Status == BSMaintenanceStatus.ModifyCurrent)
                    {
                        if (lstTabla.Where(x => x.descripcion.Trim() == strNombre.Trim() && x.arded_iid_area_descripcion != Obe.arded_iid_area_descripcion).ToList().Count > 0)
                            exists = true;
                    }
                }
                return exists;
