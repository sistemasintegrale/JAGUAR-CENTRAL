...
{
    public partial class frmManteDevolucionMaterial : DevExpress.XtraEditors.XtraForm
    {
        System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmManteDevolucionMaterial));
        ...
        public frmManteDevolucionMaterial()
        {
            InitializeComponent();
        }
        ...
        private void SetSave()
        {
            ...
            try
            {
                ...
                {
                    new BCentral().ModificarDevolucionMaterial(oBe, lstRequerimientoMaterialDetalle);
                }
            }
            catch (Exception ex)
            {
        ...
        private void modificarItem()
        {
            ...
            if (obe == null)
                return;
            using (frmManteDevolucionMaterialDetalle frm = new frmManteDevolucionMaterialDetalle())
            {
                frm.obe = obe;
                frm.lstRequerimientoMaterialDet = lstRequerimientoMaterialDetalle;
                frm.icod_almacen = Convert.ToInt32(lkpAlmacen.EditValue);
                frm.SetModify();
                frm.cod_proceso = Convert.ToInt32(lkpProceso.EditValue);
        ...
        private void lkpProceso_EditValueChanged(object sender, EventArgs e)
        {
            if (Status == BSMaintenanceStatus.CreateNew)
            {
                ...
                //grdMateriales.DataSource = lstRequerimientoMaterialDetalle.Where(obj => obj.rqmad_iproceso == Convert.ToInt32(lkpProceso.EditValue)).ToList();
                //    viewMateriales.RefreshData();
