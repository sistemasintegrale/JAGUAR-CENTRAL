...
namespace SGE.WindowForms.Otros.Ventas
{
    public partial class FrmManteNotaDebito : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void setValues()
        {
            ...
            cbIncluyeIGV.Checked = Convert.ToBoolean(oBe.ncrec_bincluye_igv);
            #endregion
            lstDetalle = new BVentas().listarNotaDebitoClienteDet(oBe.ncrec_icod_credito);
            grdNC.DataSource = lstDetalle;
            viewNC.RefreshData();
        }
        ...
        private void cargar()
        {
            ...
            BSControls.LoaderLook(lkpMotivoSUNAT, new BVentas().TablasSunatDetListar(2), "suntd_vdescripcion", "suntd_codigo", true);
        ...
        private void getNroDoc()
        {
            try
            {
                ...
                txtSerie.Text = lst[0].puvec_vserie_notaF_debito;
                txtNumero.Text = (Convert.ToInt32(lst[0].puvec_icorrelativo_nota_debito) + 1).ToString();
            }
            catch (Exception ex)
            {
        ...
        private void setSave()
        {
            ...
            try
            {
                ...
                if (Status == BSMaintenanceStatus.CreateNew)
                {
                    oBe.ncrec_icod_credito = new BVentas().insertarNotaDebitoClienteCab(oBe, lstDetalle);
                }
                else
                {
        ...
        private void setTotales()
        {
            //if (lstDetalle.Count > 0)
            //{
            //    txtMontoTotal.Text = Math.Round(lstDetalle.Sum(x => x.dcrec_nmonto_item),4).ToString();
            //    txtMontoNeto.Text = Math.Round((Convert.ToDecimal(txtMontoTotal.Text) / Convert.ToDecimal(String.Format("1.{0}", Parametros.strPorcIGV.Replace(".", "")))), 4).ToString();
            ...
            if (lstDetalle.Count > 0)
            {
                ...
                {
                    decimal total = Math.Round(lstDetalle.Sum(x => x.dcrec_nmonto_item), 4);
                    decimal igv = Convert.ToDecimal(String.Format("1.{0}", Parametros.strPorcIGV.Replace(".", ""), 4));
                    decimal neto = Math.Round(total / Convert.ToDecimal("1." + txtIgv.Text.Replace(".", "")), 4);
                ...
                {
                    decimal neto = Math.Round(lstDetalle.Sum(x => x.dcrec_nmonto_item), 4);
                    decimal igv = Convert.ToDecimal(String.Format("1.{0}", Parametros.strPorcIGV.Replace(".", ""), 4));
                    decimal total = Math.Round(neto * Convert.ToDecimal("1." + txtIgv.Text.Replace(".", "")), 4);
                    txtMontoNeto.Text = Convertir.RedondearNumero(neto).ToString();
                    txtMontoTotal.Text = Convertir.RedondearNumero(total).ToString();
                    //txtMontoIGV.Text = (Convert.ToDecimal(txtMontoTotal.Text) - Convert.ToDecimal(txtMontoNeto.Text)).ToString();
                }
        ...
        {
            ENotaDebitoDet obe = (ENotaDebitoDet)viewNC.GetRow(viewNC.FocusedRowHandle);
            if (obe == null)
                return;
        ...
        public void ListarDet()
        {
            ...
            if (lkpTipoDoc.Text == "FAV")
            {
                ...
                {
                    ENotaDebitoDet BENC = new ENotaDebitoDet();
                    ...
                    BENC.dcrec_nneto_igv = item.favd_nneto_igv;
                    ...
                    BENC.MontoAfectoImpuestoIgv = item.favd_nneto_igv;
                    ...
                    BENC.ValorUnitarioItem = Math.Round((item.favd_nneto_igv / item.favd_ncantidad_total_producto), 4);
