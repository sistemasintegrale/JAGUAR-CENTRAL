...
{
    public partial class frmListarFactVentaSuministros : DevExpress.XtraEditors.XtraForm
    {
        List<EFacturaCab> lstFactura = new List<EFacturaCab>();
        public EFacturaCab _Be { get; set; }
        public int favc_icod_cliente = 0;

        public frmListarFactVentaSuministros()
        {
            InitializeComponent();
        }
        public void cargar()
        {
            lstFactura = new BVentas().listarFacturaCab(Parametros.intEjercicio).Where(x => x.favc_icod_cliente == favc_icod_cliente && x.favc_tipo_factura == 2).ToList();
