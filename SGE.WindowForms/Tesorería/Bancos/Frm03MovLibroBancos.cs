...
namespace SGE.WindowForms.Tesorería.Bancos
{
    public partial class Frm03MovLibroBancos : DevExpress.XtraEditors.XtraForm
    {
        ...
        public List<EImagenCheque> lstImg = new List<EImagenCheque>();
        ...
        private void FrmMovLibroBancos_Load(object sender, EventArgs e)
        {
            ...
            BSControls.LoaderLook(lkpBanco, new BTesoreria().listarBancos().Where(x => x.bcoc_flag_indicador != true), "bcoc_vnombre_banco", "bcoc_icod_banco", true);
        ...
        private void adelantoProveedor()
        {
            try
            {
                ...
                FrmManteAdelantoProveedor AdelantoProveedor = new FrmManteAdelantoProveedor();
                AdelantoProveedor.Show();
                AdelantoProveedor.intIcodModulo = 6;
                ...
                AdelantoProveedor.lblCuenta.Text = lkpCuenta.EditValue.ToString();
                AdelantoProveedor.txtCuenta.Text = Convert.ToString(lkpCuenta.Text);
                AdelantoProveedor.Correlative = lstMovimientos.Count == 0 ? 1 : (lstMovimientos.Max(ob => Convert.ToInt32(ob.iid_correlativo)) + 1);
        ...
        private void adelantoCliente()
        {
            try
            {
                ...
                AdelantoCliente.intIcodModulo = 6;
                AdelantoCliente.Show();
        ...
        private void pagoAdelantoNotaCreditoCliente()
        {
            try
            {
                ...
                PagoAdelantoCliente.intIcodModulo = 6;
                PagoAdelantoCliente.Show();
        ...
        private void imprimirToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                tipoDocumento = Convert.ToInt32(Obe.ii_tipo_doc);
                if (tipoDocumento == 14)
                {
                    lstImg = new BAdministracionSistema().listarImagenesCheque(Obe.icod_correlativo);
                    rutaImg = lstImg.Select(x => x.ic_vruta).LastOrDefault();
                    var rutas = new BAdministracionSistema().listarParametro();
                    string ruta = rutas.Select(x => x.pmprc_vruta_cheque).FirstOrDefault();
                    rutaImg = ruta + rutaImg;
                }
        ...
        private void imp(ELibroBancos Obe, EVoucherContableCab oBe, List<EVoucherContableDet> lst)
        {
            ...
            reporte.cargar(Obe, oBe, lst.Where(x =>
                Convert.ToInt32(x.ctacc_iid_cuenta_contable_ref) == 0).ToList(), lkpMes.Text, lkpBanco.Text, lkpCuenta.Text, rutaImg);
        }
